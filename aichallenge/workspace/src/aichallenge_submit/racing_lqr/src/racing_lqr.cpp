...
void RacingLQR::solve(const casadi::DMDict & in, casadi::DMDict & out)
{
  ...
  auto K = casadi::DMVector(config_->N - 1, DM::zeros(1, 1));
  auto As = casadi::DMVector(config_->N - 1, DM::zeros(1, 1));
  auto Bs = casadi::DMVector(config_->N - 1, DM::zeros(1, 1));
  ...
  // simulate
  auto X_optm = DM::zeros(1, config_->N);
  X_optm(Slice(), 0) = x_ic;
  auto U_optm = DM::zeros(1, config_->N - 1);
  for (size_t k = 0; k < config_->N - 1; k++) {
    U_optm(Slice(), k) = U_ref(Slice(), k) - DM::mtimes(K[k], X_optm(Slice(), k) - X_ref(Slice(), k));
